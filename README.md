...
## Thông tin hệ thống (cập nhật)

- **Kiểu kiến trúc**: Microservices + API Gateway + RabbitMQ worker.
- **Core services đang chạy qua Compose**: `api-gateway`, `auth-service`, `user-service`, `organization-service`, `role-permission-service`, `friend-service`, `chat-service`, `task-service`, `ai-task-service`, `ai-task-worker`, `document-service`, `voice-service`, `notification-service`, `webhook-service`, `socket-service`.
- **Cổng publish ra host (mặc định)**:
  - `3000`: API Gateway (REST + proxy realtime).
  - `3005`: Voice signaling.
  - `40000-40100/tcp,udp`: WebRTC media cho `voice-service`.
- **Network nội bộ**: các service business còn lại chỉ mở trong `enterprise-network` (không publish trực tiếp ra host).
- **Quy ước cấu hình môi trường**: dùng file `.env` trực tiếp (root và từng service), không dùng luồng `ENV.example`.
...
- Tạo/cập nhật `.env` ở root repo cho biến dùng chung.
- Mỗi service có `.env` riêng trong thư mục service tương ứng (vd. `services/chat-service/.env`, `api-gateway/.env`).
- Compose đang load theo thứ tự `env_file` root trước, service sau (service override nếu trùng key).
