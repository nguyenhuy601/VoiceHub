...
export default function DeptTransferPanel({ orgId, embedded = false }) {
  ...
  const { departments, error: structureError, loadStructure } = useAdminOrgStructure(orgId, { includeInactive: embedded });
  const { members, loadMembers, membersById, error: membersError } = useAdminMembers(orgId, { view: 'directory' });
