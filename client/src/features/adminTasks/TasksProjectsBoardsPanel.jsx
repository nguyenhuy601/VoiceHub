...
export default function TasksProjectsBoardsPanel({ orgId }) {
  ...
  return (
    <AdminUserPanelShell title={t('adminDomains.tasks.projects')} hint={t('adminTasks.boardsHint')} wide>
                      ...
                      <tr key={id} className="border-b border-border/50 transition hover:bg-muted/20">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-2">
                            <Link
                              to={`/app/admin/tasks/project-team?boardId=${encodeURIComponent(id)}`}
                              className={adminSecondaryBtnClass('!px-3 !py-1.5 text-xs')}
                            >
                              {t('adminTasks.openTeam')}
                            </Link>
                            <Link
                              to={`/app/admin/tasks/delegation?boardId=${encodeURIComponent(id)}`}
                              className={adminSecondaryBtnClass('!px-3 !py-1.5 text-xs')}
                            >
              ...
              {filtered.map((board) => {
                ...
                return (
                  <div key={id} className="rounded-xl border border-border p-3">
                    <p className="font-medium text-foreground">{boardTitleOf(board)}</p>
                    ...
                    <div className="mt-3 flex flex-col gap-2">
                      ...
                      <button
                        type="button"
                        className={adminDangerBtnClass('w-full')}
                        disabled={busyId === id}
                        onClick={() => archiveBoard(board)}
                      >
                        {t('adminTasks.archive')}
