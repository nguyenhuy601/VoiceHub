...
function hasExplicitDateTime(text) {
  ...
  if (!raw) return false;
  const dateRe =
    /\b\d{1,2}[/\-.]\d{1,2}(?:\/\d{2,4})?\b|ngày\s+\d{1,2}(?:[/\-.]\d{1,2})?|hôm nay|ngày mai|ngày mốt|tuần sau|thứ\s*(2|3|4|5|6|7|bảy)|chủ nhật|today|tomorrow|next week|monday|tuesday|wednesday|thursday|friday|saturday|sunday/i;
...
export function getAiTaskEligibility(message, ctx = {}, t) {
  ...
  if (!message) {
    return { ok: false, reason: tr('aiTask.noMessage') };
  }
  const mid = message._id || message.id;
  if (!mid) {
    return { ok: false, reason: tr('aiTask.invalidMessage') };
  }
  if (message.isDeleted || message.isRecalled) {
    return { ok: false, reason: tr('aiTask.deletedOrRecalled') };
  }

  const mt = message.messageType || 'text';
  if (mt === 'system') {
    return { ok: false, reason: tr('aiTask.systemMessage') };
  }

  if (!organizationId) {
    return { ok: false, reason: tr('aiTask.needOrg') };
  }

  if (mt === 'text') {
    const text = String(message.content ?? '').trim();
    if (!text) {
      return { ok: false, reason: tr('aiTask.emptyText') };
    }
    if (!hasExplicitDateTime(text)) {
      return { ok: false, reason: tr('aiTask.needDateTime') };
    }
  }
  ...
  if (mt === 'image' || mt === 'file') {
    ...
    if (!hasFile && !hasCaption) {
      return { ok: false, reason: tr('aiTask.needFileOrCaption') };
    }
    if (!hasExplicitDateTime(caption)) {
      return { ok: false, reason: tr('aiTask.fileNeedDateTime') };
    }
  }
