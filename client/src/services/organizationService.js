...
const organizationService = {
  ...
  // Lấy orgs của user hiện tại - GET /organizations/my
  ...
  getTeams: async (orgId, departmentId) => {
    return await api.get(`/organizations/${orgId}/departments/${departmentId}/teams`);
  },

  // Tạo team - data: { name, leaderId, members }
  createTeam: async (orgId, departmentId, data) => {
    return await api.post(`/organizations/${orgId}/departments/${departmentId}/teams`, data);
  },
  ...
  updateMemberRole: async (orgId, userId, role) => {
    return await api.put(`/organizations/${orgId}/members/${userId}/role`, { role });
  },
