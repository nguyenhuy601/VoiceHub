...
/**
 * Catch-up card — heuristic hoặc AI summary qua summary-service.
 */
export default function OrgChatCatchUpCard({
  ...
  const { phase, summary, error, startSummary, reset } = useConversationSummary({
    organizationId,
    roomId,
    currentUserId,
    t,
  ...
  const previewText =
    AI_CHANNEL_CATCHUP_ENABLED && phase === 'ready' && summary?.result?.overview
  ...
  const handleViewDetails = async () => {
    if (AI_CHANNEL_CATCHUP_ENABLED && organizationId && roomId && currentUserId) {
      setModalOpen(true);
      await startSummary({ unreadOnly: true });
      onViewDetails?.();
      return;
    }
    onViewDetails?.();
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2">
              <button
                ...
                disabled={AI_CHANNEL_CATCHUP_ENABLED && phase === 'loading'}
                className="rounded-lg bg-ai px-3 py-1.5 text-xs font-semibold text-ai-foreground transition hover:bg-ai-hover disabled:opacity-60"
              >
                {AI_CHANNEL_CATCHUP_ENABLED ? t('chat.summaryAiButton') : t('chat.viewDetails')}
              </button>
              <button
                type="button"
                onClick={() => onDismiss?.()}
