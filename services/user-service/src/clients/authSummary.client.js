 ...
 * @returns {Promise<Map<string, { email?: string|null, isActive?: boolean, mustChangePassword?: boolean }>>}
 */
...
async function fetchAuthSummaryByUserId(userId) {
  const uid = String(userId || '').trim();
  if (!uid) return null;
  const map = await fetchAuthSummaryByUserIds([uid]);
