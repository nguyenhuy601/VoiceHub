...
async function createProjectRole(req, res) {
  try {
    ...
    const existingRows = await ProjectRole.find({ organizationId: orgId, projectId: null })
    ...
    const ops = orderedIds.map((id, index) => ({
      updateOne: {
        filter: { _id: id, organizationId: orgId, projectId: null },
...
async function updateProjectRole(req, res) {
  try {
    ...
    const role = await ProjectRole.findOne({ _id: roleId, organizationId: orgId, projectId: null }).lean();
    if (!role) {
      return sendServiceError(res, 404, {
        errorCode: 'PROJECT_ROLE_NOT_FOUND',
        messageUser: 'Không tìm thấy project role mặc định (org).',
        message: 'not found',
      });
    }
...
async function reorderProjectRoles(req, res) {
  try {
    ...
    await ensureOrgProjectRoles(orgId);
    const roles = await ProjectRole.find({ organizationId: orgId, projectId: null }).select('_id').lean();
    ...
    const next = await ProjectRole.find({ organizationId: orgId, projectId: null })
      .sort({ sortOrder: 1 })
      .lean();
...
async function deleteProjectRole(req, res) {
  try {
    ...
    const inUseCount = await ProjectMembership.countDocuments({
      organizationId: orgId,
