...
async function canUserRunAiPlanning(uid, orgId) {
  ...
  try {
    ...
    return { ok: true, via: 'capacity' };

  } catch {

    /* fall through — PM approver may run AI */

  }

  if (await isRequirementApproverUser(uid, orgId)) {

    return { ok: true, via: 'approver' };
...
async function resolveRequirementAccess({ userId, organizationId }) {
  ...
  const aiGate = await canUserRunAiPlanning(uid, orgId);

  const canRunAiPlanning = aiGate.ok;
    ...
    case 'requirement:run-ai-planning': {
      ...
      try {
        ...
        if (await isRequirementApproverUser(uid, orgId)) {
          ...
          return { scope, via: 'approver' };

        }

        const err = new Error(

          'Chỉ Org Admin, Resource Manager hoặc Product Manager / Project Manager / PO được chạy AI Resource Planning'
